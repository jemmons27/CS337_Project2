...
async def direct_task(ctx, task, soup, current_step, steps, ingredients, last_query):
    ...
    if re.search(navigation_re, task):
        ...
        current_step, last_query = await navigation_handler(ctx, task, steps, current_step, last_query)
    ...
    if re.search(lookup_re, task):
        ...
        current_step, last_query = await lookup_handler(ctx, task, steps, current_step, ingredients, last_query)
        return current_step, last_query
    ...
    if re.search(step_info_re, task):
        ...
        current_step, last_query = await step_info_handler(ctx, task, steps, current_step, ingredients, last_query)
