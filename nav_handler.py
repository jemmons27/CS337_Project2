...
def navigation_handler(task, steps, current_step, last_query):
    ...
    if repeat_flag:
        ...
        print(steps[current_step-1]['step'])
    ...
    if re.search(next_step, task):
        ...
        if current_step == len(steps):
            print("This is the last step, try again")
            return current_step, last_query
        ...
        if current_step == 1:
            print("There are no preceding steps, try again")
            return current_step, last_query
        current_step -= 1
        print(steps[current_step-1]['step'])
        last_query['query'] = task
    ...
    if n:
        ...
        if step.isdigit():
            ...
            for key, value in ordinal_dict.items():
                if value == step.lower():
                    n = int(key[:-2])
                    current_step = n   # Convert "1st" to 1, "2nd" to 2, etc.
                    print(steps[current_step-1]['step'])
                    last_query['query'] = task
                    last_query['output'] = steps[current_step-1]['step']
                    return current_step, last_query
