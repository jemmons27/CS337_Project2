...
"""
async def step_info_handler(ctx, task, steps, current_step, ingredients, last_query):
    ...
    if re.search(time_re, task):
        current_step, last_query = await find_time(ctx, task, steps, current_step, ingredients, last_query)
    ...
    if re.search(temperature_re, task):
        current_step, last_query = await find_temperature(ctx, task, steps, current_step, ingredients, last_query)
    ...
    if re.search(ingredients_re, task):
        current_step, last_query = await find_ingredients(ctx, task, steps, current_step, ingredients, last_query)
